...
Loaded entity mentions: 269702 Unique entity strings: 48301 Top 20 entity strings: 8193 two 7015 one 6141 three 5782 first 4260 four 3178 indian 2843 christian 2757 five 2750 english 2308 six 2243 England 1867 second 1811 seven 1663 eight 1591 ten 1527 Spain 1473 twenty 1435 Portugal 1325 half 1175 this day
...
We are going to train fine-tune spaCy to improve its ability to correctly label Early Modern text for NER.

### Step 3: Fine-tuning spaCy for LOC and GPE identification
...
**Note II:** this code will look more complex than you are used to. I am going to comment it extensively, but it is something that I have developed over time to work with hard to deal with texts (and with a healthy dose of help from GTP and Claude to help me tinker with the configuration given my data). What you need to get out of it is a sense of the steps and understanding of the data that informs what the code is doing. Once you know what you need to do and why, the actual code will make a lot more sense!
...
#
#    Each epoch:
#      a) Shuffle the training data (avoids the model learning order-
#         dependent patterns and smooths out loss spikes).
#      b) Loop over every example, convert it to a spaCy Example object,
#         and call nlp.update() to adjust the model weights.
#      c) Evaluate on the held-out dev set so we can track precision,
#         recall, and F1 — the metrics that actually matter for NER.
#      d) Save the model if it achieves the best dev F1 so far.
#####
...
for epoch in range(N_EPOCHS):
    ...
    for text, annots in TRAIN:
        # make_doc creates a blank Doc from the raw text (no predictions yet)
        doc = nlp.make_doc(text)
        # Example pairs a blank doc with gold-standard annotations so spaCy
        # can compare its predictions to the truth and compute the loss
        example = Example.from_dict(doc, annots)
        # update() runs the forward pass, computes loss, and adjusts weights
