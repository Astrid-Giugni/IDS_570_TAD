...
-   CORE (label = 1)\
    → contains *any* Tier A or Tier B word\
    (high confidence merchant discourse)

-   MAYBE (label = 2)\
    → contains Tier C words *only*\
...
What we did so far: we applied rules we designed, using the lexical triggers we chose, to create a labeled dataset. Obviously, these labels are imperfect and corpus-specific, but they are good enough to train a first classifier (that is, weak labels).
