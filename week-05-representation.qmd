...
``` r
texts_df <- tibble(doc_id = c("Circle of Commerce", "A69858"),
                   author = c("Misselden", "Unknown"),
...
anno_df <- texts_df %>%
  mutate(
    #Parse each text with Universal Dependecies parser
    anno = map(text, ~ udpipe_annotate(ud_model, x = .x) %>%
                 as.data.frame() %>%
                 rename(ud_doc_id = doc_id))   #Avoid doc_id conflic
  ) %>%
  #keep only the doc_id and he parsed annotations
  select(doc_id, anno) %>%
  #expand the nested annotations into rows
  unnest(anno) %>%
  ...
  select(
    ...
    token,
    lemma,
    upos, #parts of speech
    feats, #grammatical features (eg, tense of a verb)
    head_token_id, #what the word is dependent on (*)
...
syntax_df %>% 
  select(doc_id, token, upos, is_clause, is_dep_clause) %>%
  head(20)
```
...
sentence_df <- syntax_df %>%
  ...
  group_by(doc_id, sentence_id) %>%   #group by document and sentence
  summarise(
...
all_measures <- mls_df %>%  # ← Added mls_df %>%
  left_join(clausal_density_df %>% select(doc_id, C_per_S), by = "doc_id") %>%
  left_join(subordination_df %>% select(doc_id, DC_per_C, DC_per_S), by = "doc_id") %>%
  left_join(coordination_df %>% select(doc_id, Coord_per_C, Coord_per_S), by = "doc_id") %>%
  left_join(nominal_df %>% select(doc_id, CN_per_C, CN_per_S), by = "doc_id")
...
``` R
#| label: fig-syntactic-profile
#| fig-width: 12
#| fig-height: 8
#| fig-cap: "Complete syntactic complexity comparison"
...
# Plot
ggplot(syntax_long, aes(x = Measure, y = Value, fill = doc_id)) +
