...
``` R
texts_df <- tibble(doc_id = c("Circle of Commerce", "Mystery Text"),text = c(circle, centre))
...
anno_df <- texts_df %>%
  mutate(
    anno = map(text, ~ udpipe_annotate(ud_model, x = .x) %>%
                 as.data.frame() %>%
                 rename(ud_doc_id = doc_id))   # <- rename inside
  ) %>%
  select(doc_id, anno) %>%
  unnest(anno) %>%
  select(
...
Let's define transparent syntactic proxies: COME BACK AND FILL
...
# Clause proxy: finite verbs
is_clause = (upos %in% c("VERB", "AUX")) &
            str_detect(feats, "VerbForm=Fin"),

# Dependent clause proxy
is_dep_clause = dep_rel %in% c(
  "advcl", "ccomp", "xcomp", "acl", "relcl"
),

# Coordination proxy
is_coord = dep_rel %in% c("conj", "cc"),

# Nominal complexity proxy
is_complex_nominal = dep_rel %in% c(
  "amod", "nmod", "compound", "appos"
)

)
```

Sentence level summaries: COME BACK AND FILL
