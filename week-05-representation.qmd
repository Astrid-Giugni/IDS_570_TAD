...
An `R` coding note for those of you who are new to coding. The MTLD computation using `koRpus` is being put together in the curly brackets that define all the steps that the new function we are defining, `calculate_mtld`, will perform. **NB:** `calculate_mtld`does not actually compute MTLD, it just takes a text string (our Tidy data), converts it to a file for `koRpus`, calls in `koRpus` to compute MTLD, and then returns everything to Tidy.
